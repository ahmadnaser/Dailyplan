...
$konak = mysqli_select_db($koneksi,"db_dailyplan") or die (mysqli_error()); 
?>
...
<marquee style="color:blue" behavior="alternate" onmouseover="this.stop()" onmouseout="this.start()"><h3>Selamat datang di laporan</h3></marquee>
<hr>
...
<?php
	echo '<center><a href="hr-export-excel.php?keyword='.@$_POST['request_by'].'&tanggal='.@$_POST['tanggal'].'&tanggal2='.@$_POST['tanggal2'].' ">
	<button class="btn btn-success btn-sm">Laporan Excel</button></a>    ';
	echo '<a target="blank" href="hr-export-pdf.php?keyword='.@$_POST['request_by'].'&tanggal='.@$_POST['tanggal'].'&tanggal2='.@$_POST['tanggal2'].' ">
...
if(isset($_POST['pencarian'])){
	...
	if($tanggal !=0 and $tanggal2!=0 and $request_by != null){
		$q = "SELECT id,tgl_kunjungan, jam_kunjungan,request_by,partner1,partner2,partner3,
			...
			tgl_kunjungan between '$tanggal' and '$tanggal2' and (((request_by='$request_by' or partner1='$request_by') or 
				partner2='$request_by') or partner3='$request_by') order by tgl_kunjungan asc, jam_kunjungan asc";
	}else if($tanggal == 0 and $tanggal2==0 and $request_by == null){
		$q = "SELECT id,tgl_kunjungan, jam_kunjungan,request_by,partner1,partner2,partner3,
		...
		'$tanggal' and '$tanggal2' order by tgl_kunjungan asc, request_by asc, jam_kunjungan asc";
		//$ex = mysql_query($q);
	}
